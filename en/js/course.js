const baseUnlocked = ['pwd', 'clear', 'ls', 'cd', 'echo', 'cat', 'touch', 'mkdir', 'rm', 'type', 'which', 'man', 'help', 'grep', 'wc', 'sort', 'uniq', 'history', 'whoami', 'id', 'chmod', 'chown', 'ps', 'kill', 'top', 'printenv', 'export', 'alias', 'vi', 'apt', 'cowsay', 'df', 'du', 'ping', 'ifconfig', 'wget', 'locate', 'find', 'tar', 'gzip', 'gunzip', 'printf', 'nl', 'cut', 'paste', 'join', 'tr', 'sed', 'awk', 'gcc', 'make', 'bash', 'read', 'lpr', 'lpq', 'date', 'cal', 'file', 'less', 'free', 'ln', 'tee', 'unalias', 'apropos', 'whatis', 'info'];
...
const courseData = [
    ...
    { id: "2.1", title: "Chapter 2: Navigation - cd", description: "cd command.", text: "<code>cd</code> is used to navigate between directories. The root directory is represented by <code>/</code>.", task: "Type <code>cd /</code> to navigate to the root directory.", unlocked: ['pwd', 'clear', 'ls', 'cd'], checkCompletion: (cmd, fs) => fs.pwd() === '/' },
    ...
    { id: "34.1", title: "Chapter 34: Arrays", description: "declare -a.", text: "Arrays store multiple values inside a single variable name.", task: "Type <code>bash scripts/array-mapfile /etc/passwd</code>.", unlocked: [...baseUnlocked], checkCompletion: (cmd) => /bash\s+scripts\/array-mapfile/.test(cmd) },
