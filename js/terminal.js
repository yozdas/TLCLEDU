    ...
    executePipeline(cmdString) {
        ...
        for (let i = 0; i < pipeSegments.length; i++) {
            ...
            if (window.courseManager && !isUnlocked) {
                ...
                const hint = unlockingLesson ? ` (Bu komutu Bölüm ${this.escapeHTML(unlockingLesson.id.split('.')[0])}'de öğreneceksiniz)` : '';
                const errMsg = `bash: ${cmdName}: komut henüz öğrenilmedi${hint}`;
