...
class CourseManager {
    ...
    renderCurrentLesson() {
        ...
        document.getElementById('lesson-title').innerHTML = window.DOMPurify ? window.DOMPurify.sanitize(lesson.title) : "Hata: Güvenlik modülü eksik.";
        document.getElementById('lesson-text').innerHTML = window.DOMPurify ? window.DOMPurify.sanitize(lesson.text) : "Hata: Güvenlik modülü eksik.";
        
        const taskContainer = document.getElementById('lesson-task');
        taskContainer.innerHTML = window.DOMPurify ? window.DOMPurify.sanitize(lesson.task) : "Hata: Güvenlik modülü eksik.";
        ...
        Object.keys(chapters).sort((a, b) => parseInt(a) - parseInt(b)).forEach(chNum => {
            ...
            chapterEl.innerHTML = `
                <div class="chapter-header ${hasActive ? 'open' : ''}" onclick="this.classList.toggle('open'); this.nextElementSibling.classList.toggle('hidden')">
                    ...
                    <span class="chapter-title">${window.DOMPurify ? window.DOMPurify.sanitize(chapter.title) : "Güvenlik Hatası"}</span>
                </div>
            ...
            chapter.lessons.forEach(lesson => {
                ...
                item.innerHTML = `
                    ...
                    <div class="curriculum-details">
                        <div class="curriculum-title">${window.DOMPurify ? window.DOMPurify.sanitize(displayTitle) : "Güvenlik Hatası"}</div>
                        <div class="curriculum-desc">${window.DOMPurify ? window.DOMPurify.sanitize(lesson.description) : "Güvenlik Hatası"}</div>
                    </div>
                `;
