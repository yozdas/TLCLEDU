class CommandRegistry {
 ...
 ${line}
        ...
        this.register('jobs', (args, stdin, term) => {
            if (!window.processManager) return { error: "Süreç yöneticisi başlatılmadı." };
            const jobs = window.processManager.getJobs();
            return { output: jobs || "Aktif arkaplan süreci yok." };
        });

        this.register('kill', (args, stdin, term) => {
            if (args.length === 0) return { error: "kill: kullanım: kill [-SIGNAL] PID" };
        ...
        this.register('usermod', (args, stdin, term, fs) => {
            ...
            return { output: `Kullanıcı adı başarıyla '${newUsername}' olarak değiştirildi. Ev dizini '${newHome}' konumuna taşındı.` };
        });
    }
}
