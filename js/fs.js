class FileSystem {
    constructor() {
        ...
        const savedFs = localStorage.getItem('tlcl_fs');
        if (savedFs) {
            try {
                this.importState(savedFs);
                this.migrateFS(); // Yeni eklenen dosyaları otomatik enjekte et
            } catch (e) {
                console.error("FS yükleme hatası:", e);
                this.initDefaultFS();
            }
        } else {
            this.initDefaultFS();
    ...
    initDefaultFS() {
        ...
        // Örnek dosyalar
        this.ensureFile('readme.txt', user, 'TLCL Eğitimine Hoş Geldiniz!\nBu eğitim William Shotts\'ın kitabıyla %100 uyumludur.');
        this.ensureFile('notlar.txt', user, 'Linux her şeydir.');
...
read -p "Sayı girin: " int
if [[ "$int" =~ ^-?[0-9]+$ ]]; then
    if [ "$int" -lt 0 ]; then echo "Negatif"; else echo "Pozitif"; fi
fi`);

        this.ensureFile('read-secret', scripts, `#!/bin/bash
read -r -sp "Sifre: " pass
echo -e "\\nSifreniz: $pass"`);

        this.ensureFile('read-validate', scripts, `#!/bin/bash
read -r -p "Girdi: "
if [[ "$REPLY" =~ ^[a-zA-Z0-9]+$ ]]; then echo "Gecerli"; fi`);
...
echo "REPLY = '$REPLY'"`);
    ...
    saveState() {
        localStorage.setItem('tlcl_fs', this.exportState());
    }
